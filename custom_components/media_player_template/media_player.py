...
async def _async_create_entities(hass, config):
    ...
    for device, device_config in config[CONF_MEDIAPLAYER].items():
        ...
        browse_media_conf_entry_template = device_config.get(
                ...
                if self._browse_media_conf_entry == item:
                    ...
                    if media_content_type in [None, "library"]:
                        return await self.hass.async_add_executor_job(
                            library_payload, kodi_data["kodi"]
                        )
                    ...
                    return response
            raise BrowseError(f"Entry not found: {self._browse_media_conf_entry}")
        raise BrowseError(f"Domain not supported: {self._browse_media_domain}")
